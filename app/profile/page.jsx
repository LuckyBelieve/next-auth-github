  ...
  return (
    <div className="px-5 py-3 flex flex-col items-center justify-center">
      ...
      <div className="mt-5 flex items-center flex-col gap-3 w-[60%]">
        <img src={`${githubInfo?.avatar_url}`} alt="profile picture" className="rounded-full w-[200px] h-[200px]"/>
        <div>
          <h2 className="font-semibold text-xl opacity-80 ">{githubInfo?.username}</h2>
          <h3 className="opacity-50">{githubInfo?.login}</h3>
        </div>
        <p className=" font-light opacity-70 w-[60%] border rounded-sm">{githubInfo?.bio}</p>
